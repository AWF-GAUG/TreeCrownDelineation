...
def work(f, shpfile_path, area_max):#strtree):
    ...
    mask = xr.concat((mask, mask, mask), dim="band")
    fix_xarray_metadata(mask, [0,0,0])
    ...
    with fiona.open(shpfile_path, "r") as features:
        ...
        for f in features:
            ...
            polygon_area[1] = sobel(distance_transformed, axis=0, mode="constant")
            polygon_area[2] = sobel(distance_transformed, axis=1, mode="constant")
            norm = np.linalg.norm(polygon_area[1:],axis=0) + 1E-5
            polygon_area[1:] /= norm
            mask.loc[:, ymax_p:ymin_p, xmin_p:xmax_p] += polygon_area#np.rint(polygon_area).astype(np.uint16)
            i += 1
    ...
    mask[1:].rio.to_raster(output_file, compress="DEFLATE")
...
if __name__ == '__main__':
    ...
    #     # pool.starmap(work, zip(files, repeat(strtree, len(files))))
        pool.starmap(work, zip(files, repeat(args.shapefile, len(files)), repeat(args.area_max, len(files))))
