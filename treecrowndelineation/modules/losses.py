...
class MultiClassCrossEntropyLossWithLogits(_Loss):
    """
    Multiclass semantic segmentation cross entropy loss with logits
    """

    def __init__(self, num_classes, class_weights=None):
        super().__init__()
        self.cceloss = nn.CrossEntropyLoss(weight=class_weights)
        self.num_classes = num_classes

    def forward(self, y_pred, y_true):
        cce_loss = self.cceloss(y_pred, y_true)
        iou_loss = 1 - multiclass_jaccard_index(
            preds=y_pred,
            target=y_true,
    ...
    def forward(self, y_pred, y_true, weight_map):
        ...
        if self.reduction == "mean":
            ...
            raise NotImplementedError(
                "Reduction '{}' not implemented.".format(self.reduction)
            )
...
class LossMultiWithLogits(_Loss):
    def __init__(
        self, iou_weight=0.5, num_classes=1, class_weights=None, reduction="sum"
    ):
        super().__init__()
        self.cceloss = nn.CrossEntropyLoss(weight=class_weights, reduction=reduction)
        self.iou_weight = iou_weight
        self.num_classes = num_classes
...
def direction_loss(y_pred, y_true, mask):
    ...
    angle = torch.arccos(
        torch.clip(torch.sum(y_pred * y_true, dim=1, keepdim=True), -1 + eps, 1 - eps)
    )
